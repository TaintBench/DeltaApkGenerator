...
public class DeltaBodyTransformer extends BodyTransformer {
	...
	/**
	 * This method searches possible Jimple statements which match the given
	 * sourceInfo. First round: only compare signatures. Second round: if found more
	 * than one matches, compare line numbers. Third round: if all matches have
	 * different line numbers than sourceInfo, compare arguments.
	 *
	 * @param method
	 * @param units
	 */
	private synchronized void searchSourceStmts(SootMethod method, PatchingChain units) {
		if (!sourceInfo.isAnonymousClass)
		...
		if (canSetFinalTarget) {
			...
			if (targets.size() > 1) {
				...
				if (removed.size() == targets.size()) {
					removed.clear();
					for (Unit target : targets) {
						if (!sourceInfo.match(method, target, true, true))
							removed.add(target);
					}
				}
			}
		...
		if (targets.size() > 1) {
			if (!inserted)
				if (prevTargets.size() != targets.size()) {
					...
					for (Unit target : targets) {
						i++;
						str.append("\t" + i + ": " + target.toString() + " @lineNo."
								+ target.getJavaSourceStartLineNumber() + " @class " + targetHostClasses.get(target)
								+ "\n");
						...
						if (choice != null && useLastChoice) {
							String preStmt = null;
							if (pre != null)
								preStmt = pre.toString();
							if (choice.isSameLocation(target.getJavaSourceStartLineNumber(), target.toString(),
									preStmt)) {
								finalTarget = target;
								chosen = true;
								break;
					...
					if (useLastChoice && !chosen) {
						Main.LOGGER.info("Couldn't pick a choice automatically for flow with id " + sourceInfo.ID);
					}
					if (!chosen) {
						...
						try {
							...
							if (choice == 0) {
								...
								Main.LOGGER.info("You have chosen " + choice + ": " + finalTarget.toString()
										+ " @lineNo." + finalTarget.getJavaSourceStartLineNumber());
								if (!useLastChoice) {
									Unit pres = preStmtOfTargets.get(finalTarget);
									String preStmt = null;
									if (pres != null)
										preStmt = pres.toString();
									Location location = new Location(finalTarget.getJavaSourceStartLineNumber(),
											finalTarget.toString(), preStmt);
					...
					unit.apply(new AbstractStmtSwitch() {
						public synchronized void insert(Unit toInsert, Unit stmt) {
							if (!inserted) {
								...
								count++;
								inserted = true;
								Main.LOGGER.info(count + ". " + "Killed a flow from " + stmt + " @lineNo."
										+ stmt.getJavaSourceStartLineNumber() + "\nby inserting " + toInsert.toString()
										+ " after it.");
							}
						}
